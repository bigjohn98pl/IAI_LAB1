let nav = '<ul class="nav"><li class="nav-item"><a class="nav-link active" href="index.html">Home</a></li><li><a class="nav-link active" href="#2">Blog</a></li><li><a class="nav-link active" href="quiz.html">Quiz</a></li><li><a class="nav-link active" href="Snake/index.html">Snake</a></li><li><a class="nav-link active" href="content.html">Kontakt</a></li></ul>';
...
function move_id(value){
    ...
    MOVE_COUNT++;
    if(LAST_KEY === 37 || LAST_KEY === 39){
       POSITION_LR + POSITION_LR.push(POS);
       POSITION_LR = POSITION_LR.slice(-LENGHT_OF_PLAYER);
    }
    console.log("POS RL: "+POSITION_LR);
    PLY_TILES = document.getElementsByClassName("tile");
    for(i=0;i<POSITION.length;i++){
        PLY_TILES[POSITION[i]].classList.remove("player");
        PLY_TILES[POSITION[i]].classList.remove("player_2");
        PLY_TILES[POSITION[i]].removeAttribute("id");
    }
    ...
    for(i=0;i<POSITION.length;i++){
        PLY_TILES[POSITION[i]].classList.add("player");
        HEAD = PLY_TILES[POSITION[(POSITION.length-1)]];
        HEAD.classList.remove("player");

        if (LAST_KEY === 37) { //left
            HEAD.setAttribute("id","player_l");
            //PLY_TILES[POSITION[i]].classList.add("player_2");
            HEAD.classList.remove("player");
            HEAD.classList.remove("player_2");
        }
